...
public sealed class SettingsSerializerOrganizeTests
{
    ...
    [Fact]
    public void RoundTrips_MinimumLogLevel_AndDefaultsInformation()
    {
        IReadOnlyDictionary<string, string> stored =
            SettingsSerializer.ToStorage(new AppSettings { MinimumLogLevel = Microsoft.Extensions.Logging.LogLevel.Warning });
        SettingsSerializer.FromStorage(
            stored.ToDictionary(kv => kv.Key, kv => (string?)kv.Value), NullLogger.Instance)
            .MinimumLogLevel.Should().Be(Microsoft.Extensions.Logging.LogLevel.Warning);

        SettingsSerializer.FromStorage(new Dictionary<string, string?>(), NullLogger.Instance)
            .MinimumLogLevel.Should().Be(Microsoft.Extensions.Logging.LogLevel.Information, "the default log level");
    }

    [Fact]
    public void RoundTrips_OnCompletionCommand_AndDefaultsNull()
    {
        IReadOnlyDictionary<string, string> stored =
            SettingsSerializer.ToStorage(new AppSettings { OnCompletionCommand = @"C:\Tools\scan.exe" });
        SettingsSerializer.FromStorage(
            stored.ToDictionary(kv => kv.Key, kv => (string?)kv.Value), NullLogger.Instance)
            .OnCompletionCommand.Should().Be(@"C:\Tools\scan.exe");

        SettingsSerializer.FromStorage(new Dictionary<string, string?>(), NullLogger.Instance)
            .OnCompletionCommand.Should().BeNull("no completion command by default");
    }

    [Fact]
    public void RoundTrips_BandwidthSchedule_AndDefaultsNull()
    {
        IReadOnlyDictionary<string, string> stored =
            SettingsSerializer.ToStorage(new AppSettings { BandwidthSchedule = "22:00-06:00=0" });
        SettingsSerializer.FromStorage(
            stored.ToDictionary(kv => kv.Key, kv => (string?)kv.Value), NullLogger.Instance)
            .BandwidthSchedule.Should().Be("22:00-06:00=0");

        SettingsSerializer.FromStorage(new Dictionary<string, string?>(), NullLogger.Instance)
            .BandwidthSchedule.Should().BeNull("no schedule by default");
    }
